...
SELECT ?system ?systemLabel ?techDoc ?depicts
WHERE {
  ?system a aidoc:AISystem .
  OPTIONAL { ?system rdfs:label ?systemLabel . }
  OPTIONAL {
    ?system aidoc:hasTechnicalDocumentation ?techDoc .
    ?techDoc aidoc:depicts ?depicts .
...
SELECT ?system ?systemLabel ?techDoc ?depicts ?docType
WHERE {
  ?system a aidoc:AISystem .
  OPTIONAL { ?system rdfs:label ?systemLabel . }
  OPTIONAL {
    ?system aidoc:hasTechnicalDocumentation ?techDoc .
    ?techDoc a ?docType .
    OPTIONAL { ?techDoc aidoc:depicts ?depicts . }
